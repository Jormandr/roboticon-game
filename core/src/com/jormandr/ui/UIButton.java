...
public class UIButton {
	...
	public void draw(SpriteBatch batcher) {
		if (isPressed) {
			batcher.draw(button_textures[type2buttonIn], coords.x, coords.y, 0, 0, button_width, button_height, 4, 4,
					0);
		} else {
			batcher.draw(button_textures[type2buttonOut], coords.x, coords.y, 0, 0, button_width, button_height, 4, 4,
					0);
		}
	}
	...
	public boolean isTouchDown(int screenX, int screenY) {
		...
		float[] mousePos = GameWorld.getMousePos();
	...
	public boolean isTouchUp(int screenX, int screenY) {
		...
		if (isMouseOver(mousePos[0], mousePos[1]) && isPressed) {
			isPressed = false;
			return true;
		}
